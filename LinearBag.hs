...
union s Empty                     = s
union Empty s                     = s
union (Node x ox s) (Node y oy t) = if x<=y then Node x ox (union s (Node y oy t)) else Node y oy (union (Node x ox s) t)

intersection :: Ord a => Bag a -> Bag a -> Bag a
intersection s Empty                     = empty
intersection Empty s                     = empty
intersection (Node x ox s) (Node y oy t) | x == y = if ox<=oy then Node x ox (intersection s t) else Node y oy (intersection s t)
                                         | x > y = intersection (Node x ox s) t
                                         | otherwise = intersection s (Node y oy t)
...
difference (Node x ox s) (Node y oy t) | x==y = if ox>oy then Node x (ox-oy) (difference s t) else (difference s t) 
                                       ...
                                       | otherwise = Node x ox (difference s (Node y oy t))
